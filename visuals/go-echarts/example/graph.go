...
//gitbash
// ~/OneDrive/Desktop/BevStuff/Research/DFXM_Go/DFXM_Go/visuals/go-echarts/example = source
// C:\Users\rquon\go\src\github.com\go-echarts\go-echarts\charts = github
import (
	"encoding/json"
	"fmt"
	"log"
...
func genLinks() []charts.GraphLink {
	...
	for i := 0; i < len(graphNodes); i++ {
		for j := 0; j < len(graphNodes); j++ {
			if(rand.Intn(101) > 70){
			links = append(links,
				charts.GraphLink{	Source: graphNodes[i].Name, 
									Target: graphNodes[j].Name, 
									Symbol:"arrow", 
									SymbolSize:15, 
									ItemStyle:{borderColor:'xfff'},
								})
			}
		}
	}
